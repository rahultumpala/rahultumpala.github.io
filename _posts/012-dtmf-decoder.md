...
As we know very well, audio is a wave, and this wave produces disturbance that bounces off our eardrums, which we perceive as sound. Now, computers are discrete systems and cannot store a wave, as it is, without consuming a ton of storage. Instead, what computers do, is look at the wave as a plot of amplitude against time and sample the amplitude at regular intervals — this is the **sampling frequency/rate** — and store these amplitudes as raw data. This is known as Pulse-Code Modulation(PCM).
...
defp get_dtmf_char({high_freq, low_freq}) do
    case {high_freq, low_freq} do
      ...
      {1209, 941} -> "*"
      {1477, 941} -> "#"
       # this never happens in the context of this problem, provided the samples are generated properly, as described above.
      _ -> "."
    end
  end
```
